...
object nbs {
  ...
  def main (args:Array[String]) {
    ...
    val t_ymd = dor.slice(0, 8)
    val t_hms = dor.slice(9, 15)
    val fList = fileExists(fileFullPath,t_ymd,t_hms )
    val fExistsPath = fList (0)
    print(fExistsPath)
    val yearMonthDate = fList (1)
    val hourMinSec = fList (2)
    val year = yearMonthDate.slice(0, 4)
    val month = yearMonthDate.slice(4, 6)
    val day = yearMonthDate.slice(6, 8)
    val hour = hourMinSec.slice(0, 2)
